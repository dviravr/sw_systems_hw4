...
/*  free all trie, need to be use in case of malloc failure  */ 
...
Node* newNode(char ch, Node* parent) {
    ...
    if ((node = (Node*)malloc(sizeof(Node))) == NULL) {
        // if the malloc fail free all nodes and exit the program
        return NULL;
    }
    node->letter = ch;
    node->count = 0;
...
Node* insertChar(Node* head, Node* node, char ch) {
    ...
    if (node->children[index] == NULL) {
        if ((node->children[index] = newNode(ch, node)) == NULL) {
            // if the malloc fail free all nodes and exit the program
            freeAllAndExit(head);
        }
    } else {
